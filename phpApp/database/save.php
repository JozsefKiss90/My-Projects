...
class Usercart
{
  ...
  public function getItems($item_id = null, $table= 'cart', $table2 = 'items'){
    if (isset($item_id)){
        $result = $this->db->con->query("SELECT * FROM {$table} FULL OUTER JOIN {$table2} ON {$table}.user_id={$table2}.user_id WHERE item_id={$item_id}");
